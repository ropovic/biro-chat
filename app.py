...
def load_personnel_catalog_from_r2():
    ...
    try:
        ...
        # Strogo uzimamo samo .jpg / .png slike i izbacujemo logotipe
        ...
        catalog = []
        seen_identifiers = set() # Sprečava bilo kakvo dupliranje iste osobe
        ...
        for img_key in image_files:
            ...
            # Izvlačenje ključnog prezimena ili imena kao jedinstvenog ID-ja osobe (npr. vamovic, caldovic)
        ...
        if is_personnel_query(user_input):
            ...
            if not catalog:
                ...
                if filtered_photos:
                    ...
                    answer_text = "⚠️ U R2 bucketu ndokumenataisu pronađene odgovarajuće fotografije."
                    st.warning(answer_text)
                    st.session_state.messages.append({"role": "assistant", "content": answer_text})
            ...
            with st.spinner("Pretražujem bazu ..."):
                ...
                st.session_state.messages.append({"role": "assistant", "content": answer_text})inner("Pretražujem bazu dokumenata..."):
                result = ask_birochat(user_input)
                answer_text = result.get("answer", "Nema odgovora.")
                sources = result.get("sources", [])
